...
# import data
data <- read.delim("E16.5_vs_osmstress_E16.5.bed",sep="\t",header=T)
data2 <- read.delim("E16.5_vs_osmstress_osmstress.bed",sep="\t",header=T)
data3 <- read.delim("E16.5_vs_osmstress_osmstress.bed_E16.5.bed",sep="\t",header=T)
...
write.table(enrich_GO,"osmstress_vs_E16.5_GO_enrichment.txt",sep="\t",quote=F)
...
pdf("osmstress_vs_E16.5_GO_enrichment.pdf",height=8,width=8)
p1
dev.off()
p1
...
write.table(enrich_kegg,"osmstress_vs_E16.5_KEGG_enrichment.txt",sep="\t",quote=F)
...
pdf("osmstress_vs_E16.5_KEGG_enrichment.pdf",height=8,width=9)
p2
dev.off()
p2
